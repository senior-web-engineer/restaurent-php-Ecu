<?php
if (isset($_REQUEST['addorder_rider'])) {
session_start();
@$uid = $_SESSION['uid'];
@$role = $_SESSION['role'];
...
if (isset($_POST['order_note'])) {
    ...
    if ($store_id == '682') { //Tienda Santo Domingo
        ...
        $address = 'Matriz Santo Domingo';
        $lat = '-0.259384';
        $long = '-79.170022';
        $notes = '2';
        $total_price = number_format('0.00', 2, '.', '');
        $prod = '9407'; //id submenu
        $res_id = $_POST['store_id']; //Pide Lo Que Sea
        $user_id = $_POST['user_id_individual']; //Faster Soporte
        $order_phone = $_POST['order_phone'];
        $order_note = $_POST['order_note'];
        $assign_rider = $_POST['assign_rider']; //Asignar rider
    ...
    $sql = "insert into fooddelivery_bookorder (order_dni,order_name,order_phone,amount_pay,`id`, `assign_date_time`, `assign_status`, `timestamp_assign`, `user_id`, `res_id`, `address`, `lat`, `long`, `notes`, `total_price`, `payment`, `created_at`, `notify`, `status`, `delivery_price`, `total_general`,order_note,`is_assigned`) "
        . "values('$order_dni','$order_name','$order_phone','$amount_pay',NULL,'" . $assign_date_time . "','active','" . $assign_date_time . "','" . $user_id . "','" . $res_id . "','" . $address . "','" . $lat . "','" . $long . "','" . $notes . "','" . $total_price . "','Contra Entrega',UNIX_TIMESTAMP( DATE_FORMAT(NOW(6), '%Y-%m-%d %H:%i:%s')),'" . $notify . "','" . $status . "','" . $delivery_price . "','" . $total_general . "','" . $order_note . "','" . $assign_rider . "')";
