...
$typeStore = filter_var($_GET['type_store'], FILTER_SANITIZE_STRING);
...
if (isset($_GET['pageno'], $_GET['noofrecords'], $_GET['search'], $_GET['user_id'], $_GET['lat'], $_GET['lon'], $_GET['city'], $_GET['code'], $_GET['operative_system'], $headers['Authorization'])) {
    ...
    try {
        ...
        $consulta = $_conexion->selectAssociative("CALL restaurant_get_restaurants_by_city_filter('$search','$lat','$lon ','$city',$records,$offset, '$typeStore');");
        ...
        if ($pagecount > 0) {
            ...
            if ($page_value == 1 || ($page_value * $records) <= $pagecount) {
                ...
                $arrRecord['message'] = "Lo sentimos mucho, aún no existe tiendas disponibles o no tenemos cobertura en tu zona.";
                $arrRecord['restaurant_list'] = $consulta;
                echo json_encode($arrRecord);
                exit;
            } else {
