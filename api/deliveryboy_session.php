...
if (isset($_POST['session'], $_POST['session'], $_POST['deliverboy_id'], $_POST['code'], $_POST['operative_system'], $headers['Authorization'])) {
    ...
    if (!$verificarSesion['verify'] && $session == 'no' && $verificarSesion['success'] != '-6') {
    ...
    try {
        $consultaDelivery = $_conexion->select("SELECT  delivery_boy_session($deliverboy_id,'$session');");
        $consulta = $_conexion->select("SELECT  delivery_boy_get_presence($deliverboy_id);");
