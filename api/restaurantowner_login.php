...
if (isset($_POST['email'], $_POST['password'], $_POST['code'], $_POST['operative_system'], $headers['Authorization'])) {
 ...
 if ($token == null || strlen($token) == 0 || $token == 'null') {
        ...
        $arrRecord['message'] = 'Token de acceso inválido, vamos a asignarte un nuevo token, ingresa tus datos nuevamente';
    ...
    try {
        ...
        $consulta = $_conexion->selectAssociative("CALL restaurant_owner_login('$email','$password','$token','$so');");
        $restaurantowner_id = $consulta[0]['restaurant_ownerid'];
        ...
        $arrRecord['restaurantowner_detail'] = $consulta;
        echo json_encode($arrRecord);
        exit;
    } catch (Exception $exc) {
