...
$fullname = filter_var(ucwords(strtolower($_POST['fullname'])), FILTER_SANITIZE_STRING);
...
if (isset($_POST['fullname']) && isset($_POST['phone_no']) && isset($_POST['operative_system']) && isset($headers['Authorization']) && isset($_POST['code'])) {
    try {
        ...
        if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != "") {
            ...
            $time = time();
            $extension = obtenerExtensionImagen($_FILES['file']);
            $imagename = "profile_" . $time . '.' . $extension;
            $imageuploadpath = "../uploads/restaurant/" . $imagename;
            if (store_uploaded_image('file', $imageuploadpath, 400, 400)) {
                ...
                $arrRecord['success'] = "2";
                $arrRecord['message'] = 'Lo sentimos por el momento no podemos procesar tu solicitud al subir imagen de usuario';
            ...
            $arrRecord['success'] = "3";
            $arrRecord['message'] = 'Error de envío de parámetros, por favor envíe la imagen de usuario';
            $arrRecord["user_detail"] = array();
            echo json_encode($arrRecord);
            exit;
        }
